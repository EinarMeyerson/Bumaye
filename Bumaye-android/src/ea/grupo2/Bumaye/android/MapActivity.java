...
public class MapActivity extends FragmentActivity {
		...
		if (mapcomp != false) {
			...
			for (int i = 0; i < cofres.size(); i++) {
				...
				map.addMarker(new MarkerOptions()
						.position(
								new LatLng(cof.getLatitud(), cof.getLongitud()))
						.title("Cofre")
	...
	private void esperarLucha(String nom) {
		url = "http://" + serverAddress + ":" + serverPort
				+ "/Bumaye-api/batalla/peticion/" + personaje.getIduser() + "/"
				+ nom;
		...
		protected void onPostExecute(PeticionBatallaVO result) {
			...
			if (result != null) {
				...
				Log.i("Resultado peticion",
						"UUUUUUUUUUUUUUUli   " + result.getAceptada());
				if (result.getAceptada().equals("Si")) {
					url = "http://" + serverAddress + ":" + serverPort
							+ "/Bumaye-api/batalla/aceptarAtacante/"
							+ personaje.getIduser();
